...
function getFinals(arr) {
    const finals = arr.filter(item => item.Stage === 'Final');
    return finals;
 }
...
function getYears(arr, finalsCB) {
    return finalsCB(arr).map(item => item.Year);
}
...
function getWinnersByYear(arr, finalsCB, yearsCB, winnersCB) {
  ...
  const year = yearsCB(arr, finalsCB);
  return winners.map((item, i) => `In ${year[i]}, ${item} won the world cup!`);
}
...
function getAverageGoals(finalsCB) {
   const averageGoals = finalsCB.reduce(function(acc, item) {
